...
public class Complex {
    ...
    /**
     * Filter the listof complex numbers and return a subset
     * where the imaginary part of each complex number is zero
     *
     * @author Tyler Holewinski
     * @param nums The list of Complex numbers to filter
     * @return The list of filtered Complex numbers
     */
    public static List<Complex> getReal(List<Complex> nums) {
        return  nums.stream()
                    .filter(n -> n.imag == 0d)
                    .collect(Collectors.toList());
    }

    public static List<Complex> getSpecified(List<Complex> complex, Predicate<Complex> predicate){
        return complex.stream().filter(x -> predicate.test(x)).collect(Collectors.toList());
    }
